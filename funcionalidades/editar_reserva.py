...
def editar_reserva():
    ...
    if not reservas:
        ...
        return
    #Buscar cliente
    nombre_busqueda = input("Ingrese el nombre del cliente a editar: ").strip().lower()
    ...
    match opcion:
        case "1":
            while True:
                ...
                if validar_fecha(fecha_nueva): break
                print("[!] Formato de fecha incorrecto.")
        case "2":
            while True:
                hora_nueva = input("Ingrese la nueva hora (HH:MM): ")
                if validar_hora(hora_nueva): break
                print("[!] Formato de hora incorrecto.")
        case "3":
            while True:
                try:
                    personas_nuevas = int(input("Ingrese la nueva cantidad de personas para la reserva: "))
                    if personas_nuevas > 0: break
    ...
    for mesa, capacidad in CAPACIDAD_MESAS.items():
        if personas_nuevas <= capacidad:
            ...
            if not mesa_ocupada:
                ...
                break
    if not encontrada:
        print("[!] Error: No hay mesas disponibles o con capacidad suficiente para esta reserva")
